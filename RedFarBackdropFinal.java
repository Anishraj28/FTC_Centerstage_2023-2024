...
@Autonomous(name = "RedFarBackdropFinal", group = "Linear Opmode")
// @TeleOp(...) is the other common choice
// @Disabled
public class RedFarBackdropFinal extends LinearOpMode {

    // Declare Devices
    private CompetitionBlueLeft detector;
    ...
    public void runOpMode() {
        ...
        detector = new CompetitionBlueLeft(telemetry);
...
*/
       ...
       /*
        if (distanceright.getDistance(DistanceUnit.INCH) < 10 && distanceright.getDistance(DistanceUnit.INCH) < 35) {
            sleep(1000);
...
/*
        if (distanceright.getDistance(DistanceUnit.INCH) < 25 && distanceleft.getDistance(DistanceUnit.INCH) >= 20) {
...
//        //--------------------------------------------------------
        ...
        turnClockwise(70, medium); //adjusting for 90 degree angle
        //move forward to deposit purple pixel on right line
        moveForward(5, medium);
            //ARM CODE HERE
            servoOpen(servoarm, -0.2);
            // open servo to deposit pixel
        //servoOpen(servoarm);
        servoarm.setPosition(0.7);
        //strafe right to align with backstage area
        moveRight(14, medium);
        //move forward to park robot in backstage
        moveForward(51, medium);
        //turn robot around to prepare for teleop
        turnClockwise(165, medium);
        }
        else if(distanceleft.getDistance(DistanceUnit.INCH) < 25 && distanceright.getDistance(DistanceUnit.INCH) >= 20) {
...
//        //move back to park in backstage
        ...
        else if (distanceright.getDistance(DistanceUnit.INCH) >= 20 && distanceleft.getDistance(DistanceUnit.INCH) >= 20){

            //--------------------------------------------------------
            // autonomous for when robot is far from the backdrop, CENTER LINE
            //--------------------------------------------------------
            sleep(1000);
            //move forward to center line clearly
            moveForward(13, medium);
            //deposit purple object here with arm
            //ARM CODE HERE
            // open servo to deposit pixel
            //servoOpen(servoarm);
            servoarm.setPosition(0.7);
            //moveArmMotor(10, medium); // do we need this??
            //slideMotorUpTest(8, medium);
